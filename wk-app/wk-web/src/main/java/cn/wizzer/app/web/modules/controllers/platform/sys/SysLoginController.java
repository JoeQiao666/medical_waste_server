    ...
    public Object doLogin(@Attr("loginToken") AuthenticationToken token, HttpServletRequest req, HttpSession session) {
        ...
        try {
            ...
            if (!user.getLoginname().equals("superadmin")){
                 if (Strings.isEmpty(user.getPermission()))return Result.error("无权限");
            }
            ...
            Sql sql = Sqls.create("select * from user_with_other where id='"+user.getId()+"'");
            sql.setCallback(Sqls.callback.maps());
            userService.dao().execute(sql);
            return Result.success("login.success",sql.getList(NutMap.class).get(0));
